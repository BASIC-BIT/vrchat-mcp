...
| npm                   | Live   | `@basicbit/vrchat-mcp`                           | Latest verified release: `0.1.11`.                                      |
| Official MCP Registry | Live   | `io.github.BASIC-BIT/vrchat-mcp`                 | Latest verified release: `0.1.11`; package entry points to npm `0.1.11`. |
| GitHub Releases       | Live   | https://github.com/BASIC-BIT/vrchat-mcp/releases | Latest verified release: `v0.1.11`.                                     |
...
- Safer: local stdio, loopback-only authenticated Streamable HTTP, MCPB, Claude/Codex plugins that launch local stdio, and client configs that run `@basicbit/vrchat-mcp` on the user's machine.
...
- Public Streamable HTTP URL, which remains inappropriate for user-authenticated VRChat access. The native HTTP mode is deliberately loopback-only and is not a Smithery remote endpoint.
