...
**Open question, needs a deliberate decision.** `releaseStatus` is the weakest part of that argument. Flipping an avatar to public is the avatar analogue of `publishWorld` / `unpublishWorld`, which this policy blocks outright. If the guideline is read as covering publication rather than only upload, `releaseStatus` belongs in the blocked set and the tool should narrow to name, description and tags.
