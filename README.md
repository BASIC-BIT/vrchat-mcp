...
- `mcp:login` / `mcp:status` / `mcp:logout` - local harness for VRChat auth
- `vrctl:login` / `vrctl:status` / `vrctl:logout` - local harness for vrc.tl (Timeline) auth
...
```json
{
  ...
  "vrctl": { "enabled": true, "auth": { "cookieStore": "file" } },
  "writes": { "allow": false },
...
- `VRCHAT_MCP_VRCTL_ENABLED` (optional): enable vrc.tl tools (`true|1|yes|on`). Default is disabled.
- `VRCHAT_MCP_VRCTL_SITE_URL` (optional): override vrc.tl site URL (default `https://vrc.tl`).
- `VRCHAT_MCP_VRCTL_API_BASE_URL` (optional): override vrc.tl API base (default `https://vrc.tl/api/v1`).
- `VRCHAT_MCP_VRCTL_USER_AGENT` (optional): override user-agent sent to vrc.tl.
- `VRCHAT_MCP_VRCTL_COOKIE_STORE` (optional): cookie store for vrc.tl (`memory|file|keychain`). Defaults to `VRCHAT_MCP_COOKIE_STORE` when unset.
- `VRCHAT_MCP_VRCTL_COOKIE_FILE` (optional): cookie file for vrc.tl when store is `file` (default `~/.vrchat-mcp-vrctl-cookies.json`).
